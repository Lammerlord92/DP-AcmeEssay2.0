...
<display:table name="essays" id="row" requestURI="${requestURI}" pagesize="5" class="displaytag">
	...
	<display:column title="action">
		<security:authorize access="hasRole('AUTHOR')">
			<a href="essay/author/details.do?essayId=${row.id}">
			<spring:message code="essay.details"/></a> 
			<br/>
			<a href="publicSession/author/listByEssay.do?essayId=${row.id}">
			<spring:message code="essay.session"/></a> 
			<br/>
		</security:authorize>
		...
		<security:authorize access="isAnonymous()">
			<a href="essay/details.do?essayId=${row.id}">
			<spring:message code="essay.details"/></a> 
			<br/>
		</security:authorize>
	</display:column>
